...
class CoverageResults:
    ...
    def write_results(self, show_missing=True, summary=False, coverdir=None):
        ...
        if self.calledfuncs:
            ...
            print("functions called:", file=sys.stderr)
            calls = self.calledfuncs
            for filename, modulename, funcname in sorted(calls):
                print(("filename: %s, modulename: %s, funcname: %s"
                       % (filename, modulename, funcname)), file=sys.stderr)

        if self.callers:
            print("", file=sys.stderr)
            print("calling relationships:", file=sys.stderr)
            lastfile = lastcfile = ""
            for ((pfile, pmod, pfunc), (cfile, cmod, cfunc)) \
                    in sorted(self.callers):
                if pfile != lastfile:
                    print("", file=sys.stderr)
                    print("***", pfile, "***", file=sys.stderr)
                    lastfile = pfile
                    lastcfile = ""
                if cfile != pfile and lastcfile != cfile:
                    print("  -->", cfile, file=sys.stderr)
                    lastcfile = cfile
                print("    %s.%s -> %s.%s" % (pmod, pfunc, cmod, cfunc), file=sys.stderr)
        ...
        if summary and sums:
            print("lines   cov%   module   (path)", file=sys.stderr)
            for m in sorted(sums):
                n_lines, percent, modulename, filename = sums[m]
                print("%5d   %3d%%   %s   (%s)" % sums[m], file=sys.stderr)
...
class Trace:
                    ...
                    if not ignore_it:
                        if self.trace:
                            print((" --- modulename: %s, funcname: %s"
                                   % (modulename, code.co_name)), file=sys.stderr)
    ...
    def localtrace_trace_and_count(self, frame, why, arg):
        if why == "line":
            ...
            if self.start_time:
                print('%.2f' % (_time() - self.start_time), end=' ', file=sys.stderr)
            bname = os.path.basename(filename)
            print("%s(%d): %s" % (bname, lineno,
                                  linecache.getline(filename, lineno)), end='', file=sys.stderr)
        return self.localtrace
